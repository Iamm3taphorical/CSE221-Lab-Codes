# PROBLEM: Build and print adjacency matrix for directed weighted graphs
# INPUT: Multiple test cases, each with N vertices, M edges, and edge triplets (u, v, weight)
# OUTPUT: N x N matrix where matrix[i][j] = weight of edge from vertex i+1 to j+1 (0 if no edge)
# KEYWORDS: adjacency matrix, directed graph, weighted edges, graph representation

import sys  # access fast stdin/stdout

input = sys.stdin.readline  # faster line-based input

# read number of test cases from the first line
t_var = int(input())
